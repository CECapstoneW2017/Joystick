...
static int protothreadAngle(struct pt *pt, int interval) {
  ...
  while(1) { // never stop 
    ...
    *  argument "micros() - timestamp > interval" is re-evaluated
    *  and if false the function exits after that. */
    PT_WAIT_UNTIL(pt, micros() - timestamp > interval );
    timestamp = micros(); // take a new timestamp
...
static int protothreadInput(struct pt *pt, int interval) {
  ...
  PT_BEGIN(pt);
  while(1) {
    PT_WAIT_UNTIL(pt, (Serial.available() > 0) && (micros() - timestamp > interval) ); // What should the condition be?
    timestamp = micros();
    //Read from the serial input buffer
    inputString = Serial.readStringUntil('\n');
    inputString.toCharArray(str,255);
    //Process Strings to get each motorTorquePercent
    sscanf(str,"%i-%i", &newTorque2, &newTorque1);
  }
  PT_END(pt);
}
...
void loop() {
  ...
  protothreadAngle(&pt1, 400);     //Angle Proto-Thread
  protothreadInput(&pt3, 400);     //Serial Input Proto-Thread
  protothreadMotor(&pt2);         //Motor Proto-Thread
  protothreadOutput(&pt4, 400);  //Serial Output Proto-Thread
}
